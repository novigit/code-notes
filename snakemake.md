...
```sh
If you have a rule with `conda: env.yaml`, snakemake will download and install the requested packages under `.snakemake/conda/<env>` where `<env>` is an MD5 hash. The rule will be run with the software installed there.
```
